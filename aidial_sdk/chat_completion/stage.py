...
class Stage:
    ...
    def append_content(self, content: str):
        if not self._opened:
            runtime_error(
                self._log, "Trying to append content to an unopened stage"
            )
        if self._closed:
            runtime_error(
                self._log, "Trying to append content to a closed stage"
            )
    ...
    def append_name(self, name: str):
        if not self._opened:
            runtime_error(
                self._log, "Trying to append name to an unopened stage"
            )
        if self._closed:
            runtime_error(self._log, "Trying to append name to a closed stage")
    ...
    def add_attachment(
        ...
        if not self._opened:
            runtime_error(
                self._log, "Trying to add attachment to an unopened stage"
            )
        if self._closed:
            runtime_error(
                self._log, "Trying to add attachment to a closed stage"
            )
